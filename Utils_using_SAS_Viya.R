...
lecture_tables_aggregation <-function(Freq, Mois, RevFin, PeriodeFin, Repertoire, ConnectionSecureDB)
{
  ...
  if(RevFin == "KI")
  {
    if(Freq =="M")
    {
      ...
      else
      {
        #case 2
        CheminData <- CheminDataset[-c(10,11,12)]
        RequetesSQL <- paste0("SELECT * FROM ",CheminData, " WHERE PeriodeFin =", Mois)
      }
    }
    else
    {
      if(ConditionSpeciale)
      {
        #case 3
        CheminData <- CheminDataset[-c(10,11,12)]
        RequetesSQL <- paste0("SELECT * FROM ",CheminData, " WHERE scan(code, 15) <> '_X'")
      }
      else
      {
        #case 4
        CheminData <- CheminDataset[-c(10,11,12)]
        RequetesSQL <- paste0("SELECT * FROM ", CheminData)
      }
    }
  }
  else
  ...
  print(paste("TABLES AGREGATION LU DE", RequetesSQL))
  ...
  for(j in 1:length(RequetesSQL))
  {
    ...
    Liste_Table <- rlist::list.append(
      Liste_Table, read_csv2(name_table, guess_max = 12000000))
  }
  ...
  AggregationTouteCategorie =
    AggregationTouteCategorie[, montant:= as.numeric(montant)]
  AggregationTouteCategorie =
    AggregationTouteCategorie[, CONF_STATUS:= as.character(CONF_STATUS)]
...
lecture_tables_mixte <-function(Freq, Mois, RevFin, PeriodeFin, Repertoire, ConnectionSecureDB)
{
  ...
  if(ConditionSpecialeMixte)
  {
    if(Freq == "M")
    {
      RequetesSQLIdMixte <- paste0("SELECT * FROM ", CheminDatasetMixte, " WHERE PeriodeFin =", "'", Mois, "'", " AND scan(code,15,'.') = _X' AND scan(code,11,'.') = 'F5A'")
    }
    else
    {
      RequetesSQLIdMixte <- paste0("SELECT * FROM ", CheminDatasetMixte, " WHERE scan(code,15,'.') = _X' AND scan(code,11,'.') = 'F5A'")
...
{
  # ReferencePiZones devient une refence a une CASTable reference en entree suite à l'import en memoire des tables
  ...
  #Query <- paste0("SELECT * FROM ", "'", CheminReferentielPays, "'", "WHERE (CODE_PAYS IS NOT NULL OR CODE_PAYS <> '')")
  
  # TableCodePays <- data.table::data.table(traite_espace_chaine_caractere(secureDB::getQuery(connectionId = ConnectionSecureDB, query = Query)))
  TableCodePays <- data.table::data.table(to.data.frame(to.casDataFrame(ReferencePiZones['code_pays'<>'',])))
...
application_plan_parametrage_aggregation_code_serie_mensuel <-function(AggregationMensuelleTouteCategorie, PlanAggregationParametre)
{
  print("application_plan_parametrage_aggregation_code_serie_mensuel monostream")
  print(Sys.time())
  
  #  nÃ©cessaire pour la comparaison des montants
  # Ã  la fin du code
  AggregationMensuelleTouteCategorie =
    AggregationMensuelleTouteCategorie[, montant:=as.double(montant)]
...
import_all_csv_in_memory <- function(casconn, inputcaslib, outputcaslib) {
  ...
  for(i in 1:length(listfiles$FileInfo$Name)){
    ...
    if ((grepl('.csv',file_name)) & !(grepl('creditcard',file_name))) {
      ...
      cas.table.loadTable(conn, casout=list(caslib=outputcaslib,name=table_name), caslib=inputcaslib, path=file_name, importoptions=list(delimiter=';',filetype='csv',guessRows=10000,getnames=TRUE,varchars=TRUE,stripblanks=TRUE))
    }
  }
}
...
cas_append2tables <- function(casconn, inputcaslib, inputcastab, outputcaslib, outputcastab) {
  codeds <- paste0('data "', outputcastab, '"(caslib="', outputcaslib , '" append=yes replace=yes); set "',inputcastab,'"(caslib="', inputcaslib, '"); run;')
...
# ######################################################################################################
# Fonction permettant de sauvegarder une table CAS dans la datasource associée à la librairie CAS (Filesystem, Hadoop, Oracle,etc...)
